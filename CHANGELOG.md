...
- **Rich Formatting Toolbar in Iframe**: Integrated a rich text editing toolbar (Bold, Italic, Underline, Alignment, Font Family, Font Size) right above the selected element inside the canvas iframe. It automatically detects and supports inline editing for any block containing text (such as Título, Párrafo, Botón, Cita, Acordeón, Tarjeta de Producto) using a dynamic `data-prop` attribute, propagating changes back to the parent and compiler.
